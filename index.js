...
class Song {
  constructor(file, serv, options) {
    ...
    if (options.hasOwnProperty("tempo")) {
      this.tempoOffset = options.tempo;
    }

    this.repeating = false;
    if (options.hasOwnProperty("repeat")) {
      this.repeating = options.repeat;
    }
  ...
  play() {
    this.serv.on("tick", (e) => {
      this.currentSong.Notes.forEach((item, i) => {
        if (
          i * this.tempo ==
          Math.floor(this.serv.tickCount - this.currentTick)
        ) {
          item.forEach((inst, t) => {
            if (inst.Inst <= this.instruments.length) {
              this.currentPitch =
                63 * Math.pow(2, (inst.Key - 33 + inst.Pitch / 100 - 12) / 12);
          ...
          if (i >= this.currentSong.Length - 1 && this.repeating == true) {
